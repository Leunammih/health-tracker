...
- NEVER EMIT A NUMBER THE USER DID NOT GIVE. This matters most for wellbeing.energy, wellbeing.mood and day_context.stress_load: set them ONLY when the user states a number ("energy 6", "mood was a seven") or gives a verbal rating that maps to one unambiguously ("energy was terrible", "completely calm today"). Do NOT derive them from the tone of the entry, from how the day sounds, or from what usually goes with what — a low-sounding day is not a stated mood of 4. If one is missing and the entry implies a full day, leave the field out and ASK for it in follow_up_questions instead; that is exactly what those questions are for. The user often enters these by hand on a slider before dictating, and a guessed number lands on top of the real one.
